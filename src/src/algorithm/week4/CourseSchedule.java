...
public class CourseSchedule {
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int[] prerequisite : prerequisites) {
            neighbors.computeIfAbsent(prerequisite[0], k -> new ArrayList<>());
            neighbors.get(prerequisite[0]).add(prerequisite[1]);
        }

        while (start < numCourses) {
            if (!checkWithNeighbors(start)) {
                return false;
            }
            start++;
        }
    ...
    public boolean checkWithNeighbors(int start) {
        if (finished[start]) {
            return true;
        } else {
            if (visited[start]) {
        ...
        if (neighbors.get(start) == null) {
            return finished[start] = true;
        }
        for (Integer neighbor : neighbors.get(start)) {
            if (!checkWithNeighbors(neighbor)) {
                return false;
            }
        }
    ...
    public int[][] buildAdjacencyMatrix(int[][] prerequisites, int numCourses) {
        int[][] result = new int[numCourses][numCourses];
        for (int[] prerequisite : prerequisites) {
            result[prerequisite[0]][prerequisite[1]] = 1;
        }
        return result;
    }

    public Map<Integer, ArrayList<Integer>> buildNeighbors(int[][] prerequisites, int numCourses) {
        Map<Integer, ArrayList<Integer>> result = new HashMap<>();
        for (int[] prerequisite : prerequisites) {
            result.computeIfAbsent(prerequisite[0], k -> new ArrayList<>());
    ...
    public static void main(String[] args) {
        ...
        System.out.println(courseSchedule.canFinish(100, prerequisites));
